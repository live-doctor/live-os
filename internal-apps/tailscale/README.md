...
This folder packages the Tailscale daemon as an internal Homeio app.
...
2. Authenticate:
   - Easiest: `docker exec -it homeio-tailscale tailscale up`
   - Optional unattended: set `TS_AUTHKEY` env in the compose and redeploy.
3. Verify: `docker exec -it homeio-tailscale tailscale status`
...
- Container name: `homeio-tailscale`; state volume: `tailscale-state`.
